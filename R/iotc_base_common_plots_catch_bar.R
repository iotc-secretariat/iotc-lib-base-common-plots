...
#'@examples catch_bar(NC_est(), C_SPECIES_CODE)                   # Bar chart with yearly estimated catches by species code
#'@examples catch_bar(NC_est(), C_FLEET_CODE, max_categories = 5) # Bar chart with yearly estimated catches by fleet code, limited to the first five fleets in terms of total catches
#'@examples catch_bar(NC_raw_std(years = c(2000:2010), species_group_codes = "TUNAS"), C_SPECIES_CODE, colors = color_table(unique_colors(16))) # Bar chart with yearly (2000-2010) raw catches by species code (for tuna species) with colors re-assigned
#'@export
